...
Essa característica da linguagem é usada deliberadamente para garantir que **somente o agregado `Cofre` pode mutar as entidades do domínio**.
...
Mutações só são possíveis de dentro de `internal/vault`, onde o agregado `Cofre` opera.
...
`Cofre` é o agregado raiz. Todo acesso de mutação ao domínio passa por ele — mas ele não concentra toda a lógica de negócio. A responsabilidade é dividida entre o `Cofre` e as entidades, seguindo um critério claro.
...
### Divisão de responsabilidades: Cofre vs. Entidades

O critério que determina onde um método vive:

> **O receiver é a entidade mais próxima que consegue validar todos os invariantes da operação sozinha, sem precisar de referência externa. Quando a operação cruza fronteiras ou afeta estado global, fica no `Cofre`.**
...
| Mover pasta | `Cofre` | `Cofre` | ✅ ciclo + duas pastas |
| Excluir pasta | `Cofre` | `Cofre` | ✅ promove filhos para pai |
| Criar modelo | `Cofre` | `Cofre` | ✅ unicidade global |

### O padrão: público no Cofre, privado na entidade

Métodos públicos vivem no `Cofre` — são a API que o Manager chama. A lógica local (validação de invariantes locais, construção da entidade) vive em métodos privados das entidades, acessíveis dentro do pacote `internal/vault`.

O `Cofre` delega a lógica local para a entidade e mantém o estado global: `modificado` e `dataUltimaModificacao`.

```go
// internal/vault/cofre.go — método público, coordenador
func (c *Cofre) CriarSegredo(pasta *Pasta, nome string, campos []CampoSegredo) (*Segredo, error) {
    segredo, err := pasta.criarSegredo(nome, campos) // delega lógica local à Pasta
    if err != nil {
        return nil, err
    }
    c.modificado = true                        // estado global atualizado pelo Cofre
...
Se as entidades mutassem diretamente sem passar pelo `Cofre`, não haveria ponto central para rastrear se o cofre tem alterações não salvas. O Manager precisa saber se deve oferecer "Salvar" ao usuário, e o `Cofre` é o único que tem visão de todas as mutações ocorridas na sessão.
...
TUI → Manager.RenomearSegredo(segredo, "novo nome")
         ↓
      cofre.RenomearSegredo(segredo, "novo nome")
         ↓ validação (unicidade de nome na pasta pai)
         ↓ mutação do estado interno
         ↓ cofre.modificado = true
         ↓ atualização de dataUltimaModificacao
         ↓ retorna erro ou nil
...
// Cofre — usa o feedback para decidir o que atualizar
func (c *Cofre) RenomearSegredo(segredo *Segredo, novoNome string) error {
    alterado, err := segredo.pasta.renomearSegredo(segredo, novoNome)
    if err != nil {
        return err
    }
    if alterado {
        segredo.marcarModificado()         // estadoSessao → modificado
        c.modificado = true
        c.dataUltimaModificacao = time.Now()
    }
    return nil
...
```
TUI  →  Manager (API pública)  →  Cofre (coordenador)  →  Entidades (lógica local)
         único ponto de            atualiza estado          métodos privados,
         entrada para              global e delega          acessíveis apenas
         toda mutação                                       dentro do pacote
```
...
- Adicionar uma nova operação significa sempre: novo método no Manager + novo método no Cofre + (se necessário) novo método privado na entidade
