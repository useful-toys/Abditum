...
// TestExcluirModelo verifies templates can always be deleted without restrictions.
func TestExcluirModelo(t *testing.T) {
	cofre := NovoCofre()
	manager := NewManager(cofre, nil)
	...
	if err != nil {
		t.Errorf("Expected no error deleting template, got: %v", err)
	}

	// Verify template was removed
	if len(cofre.Modelos()) != 0 {
		t.Errorf("Expected 0 templates after deletion, got %d", len(cofre.Modelos()))
	}
}
