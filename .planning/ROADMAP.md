...
- [x] 03-04-PLAN.md � Template Management: CriarModelo, RenomearModelo, ExcluirModelo with in-use check, field operations (add/remove/reorder), alphabetical ordering, "Observa��o" prohibition
...
**UAT:**
- [ ] `Manager.Create` initializes vault with Pasta Geral + "Sites e Apps" + "Financeiro" subfolders + 3 default templates (Login, Cart�o de Cr�dito, Chave de API)
...
- [ ] `CreateTemplateFromSecret` excludes ALL fields named 'Observa��o' (both auto-Observation and any user-named field with that name); a template produced from such a secret never contains a field named 'Observa��o'
- [ ] `UpdateTemplateStructure` returns explicit error when attempting to add or rename a field to 'Observa��o'
...
Plans:
- [x] 04.1-01-PLAN.md — Entity private methods: state management (marcarModificado/marcarModificacao), wipe (zerarValoresSensiveis), factory estadoSessao fix, deep copy (copiarProfundo)
- [x] 04.1-02-PLAN.md — Serialization versioning: DeserializarCofre(version), remove migrate.go and its tests
- [x] 04.1-03-PLAN.md — Manager refactoring: replace direct field access with entity methods, fix AlternarFavorito bug, add regression tests
...
- [x] AlternarFavoritoSegredo does NOT update segredo.DataUltimaModificacao() — only cofre.DataUltimaModificacao() changes
...
- [x] 05.7-02-PLAN.md — MessageBar golden tests: 6 kinds × 2 widths = 24 files (Wave 2)
- [x] 05.7-03-PLAN.md — CommandBar golden tests: 5 scenarios × 2 widths = 20 files (Wave 2)
...
Plans:
- [ ] 06.2-01-PLAN.md — root.go: corrigir diálogos Ctrl+Q (desvios 1 e 2) + implementar Fluxo 6 (Ctrl+Alt+Shift+Q)
- [ ] 06.2-02-PLAN.md — flow_open_vault.go: dirty-check Decision + erros senha/arquivo Acknowledge (desvios 3, 4, 5)
- [ ] 06.2-03-PLAN.md — flow_create_vault.go + flow_save_and_exit.go: desvios 6, 7, 8, 9 (dirty-check, overwrite, senha fraca, conflito externo + N Salvar como novo)
- [ ] 06.2-04-PLAN.md — help.go: formatKeyForHelp() + aplicar em buildContentLines() (desvio 10)
...
**UAT:**
- [ ] Creating a secret from "Login" template produces fields: URL, Usu�rio, Senha (sensitive), Observa��o (auto, last)
- [ ] Creating a secret with no template produces only Observa��o field
