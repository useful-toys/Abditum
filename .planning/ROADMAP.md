...
### Phase 04.1: Refinamento da camada de domínio — encapsulamento e versioning (INSERTED)

**Goal:** Corrigir desvios de encapsulamento identificados em revisão pós-fase 4 e adicionar suporte a versões de formato no deserializador; nenhuma funcionalidade nova — apenas alinhamento da implementação com os princípios arquiteturais documentados.
...
Plans:
- [ ] 04.1-01-PLAN.md — Entity private methods: state management (marcarModificado/marcarModificacao), wipe (zerarValoresSensiveis), factory estadoSessao fix, deep copy (copiarProfundo)
- [ ] 04.1-02-PLAN.md — Serialization versioning: DeserializarCofre(version), remove migrate.go and its tests
- [ ] 04.1-03-PLAN.md — Manager refactoring: replace direct field access with entity methods, fix AlternarFavorito bug, add regression tests

**UAT:**
- [ ] CriarSegredo returns secret with estadoSessao == EstadoIncluido (not EstadoModificado)
- [ ] DuplicarSegredo returns duplicate with estadoSessao == EstadoIncluido
- [ ] AlternarFavoritoSegredo does NOT update segredo.DataUltimaModificacao() — only cofre.DataUltimaModificacao() changes
- [ ] DeserializarCofre(data, version) new signature compiles and all existing tests pass
- [ ] internal/storage/migrate.go does not exist; go test ./... green
