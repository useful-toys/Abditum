...
- [Golden Files: Dois Níveis de Validação](#golden-files-dois-níveis-de-validação)
  - [Nível 1 — `.golden.txt`: Layout e Estrutura](#nível-1--goldentxt-layout-e-estrutura)
  - [Nível 2 — `.golden.json`: Estilos e Cores](#nível-2--goldenjson-estilos-e-cores)
...
- [Testes Comportamentais com Dados da `golden.json`](#testes-comportamentais-com-dados-da-goldenjson)
...
│       ├── {component}-{variant}-{size}.golden.txt
│       └── {component}-{variant}-{size}.golden.json
...
### Nível 1 — `.golden.txt`: Layout e Estrutura

O arquivo `.golden.txt` armazena o output visual do componente com **todos os códigos ANSI removidos**, produzindo texto puro. É o "desenho" canônico da tela — bordas, spacing, quebras de linha e conteúdo textual.
...
### Nível 2 — `.golden.json`: Estilos e Cores

O arquivo `.golden.json` armazena as **transições de estilo visual** extraídas do output ANSI original, no formato de um array JSON de tuplas compactas. Cada tupla registra linha, coluna, cor de foreground, cor de background e atributos de fonte no ponto onde o estado visual muda.
...
```
{component}-{variant}-{size}.golden.{ext}
```
...
|---|---|---|---|
| `messages-success-30.golden.txt` | message bar | success | 30 colunas |
| `decision-destructive-1action-short-60x24.golden.json` | decision dialog | destrutivo, 1 ação, título curto | 60×24 |
| `help-15actions-bottom-30x16.golden.txt` | help modal | 15 ações, scroll no fim | 30×16 |
| `filepicker-open-withfiles-80x24.golden.json` | file picker | modo open, com arquivos | 80×24 |
| `welcome-cyberpunk-80.golden.txt` | welcome screen | tema cyberpunk | 80 colunas |
...
O resultado é que os arquivos `.golden.json` podem ser comparados byte-a-byte sem falsos positivos causados por refatorações que preservam o output visual mas alteram os códigos SGR internos.
...
|---|---|---|
| `goldenPath(component, variant, width, ext)` | `messages_test.go` | Padrão genérico `{component}-{variant}-{width}.golden.{ext}` |
...
## Testes Comportamentais com Dados da `golden.json`
