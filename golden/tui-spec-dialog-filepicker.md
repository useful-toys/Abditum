...
**Modo Open — arquivo selecionado (ação default ativa, scroll em ambos os painéis):**
...
**Modo Open — nenhum arquivo (ação default bloqueada, sem scroll):**
...
**Modo Save — campo nome preenchido (ação default ativa):**
...
**Modo Save — campo nome vazio (ação default bloqueada):**
...
**Tokens exclusivos do Modo Save:**
...
| Tecla | Efeito |
|---|---|
| `↑` / `↓` | Navega entre pastas visíveis |
| `→` | Expande pasta recolhida |
| `←` | Recolhe pasta expandida |
| `Enter` | (Open) Avança foco para o primeiro arquivo no painel de arquivos, se a pasta contém `<ext>`; sem efeito se não contém |
| `Enter` | (Save) Idem ao Open |
| `Home` / `End` | Vai ao primeiro / último item visível |
| `PgUp` / `PgDn` | Scroll por página |

**Navegação no painel de arquivos:**

| Tecla | Efeito |
|---|---|
| `↑` / `↓` | Navega entre arquivos |
| `Enter` | (Open) Confirma seleção — equivale à ação default |
| `Enter` | (Save) Copia nome (sem extensão) para o campo `Arquivo:` e move foco para o campo — **não** confirma o diálogo |
| `Home` / `End` | Vai ao primeiro / último arquivo visível |
| `PgUp` / `PgDn` | Scroll por página |

**Navegação no campo `Arquivo:` (Modo Save):**

| Tecla | Efeito |
|---|---|
| Texto / Backspace / Delete | Edita o nome do arquivo |
| `Enter` | Confirma — diálogo fecha com caminho completo (diretório + nome + `<ext>`) |
...
**Comportamento compartilhado (ambos os modos):**
...
- **Duplo-clique em arquivo:** (Open) confirma seleção; (Save) copia nome para o campo `Arquivo:`
...
**Comportamento exclusivo do Modo Open:**

- **Ordem do Tab:** Árvore → Arquivos → volta (2 stops)

**Comportamento exclusivo do Modo Save:**
