...
- **Global scope**: `C:/git/Abditum/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/git/Abditum/.gemini/notes/`)
...
2. Glob `C:/git/Abditum/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/git/Abditum/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/git/Abditum/.gemini/notes/` — works in any directory
